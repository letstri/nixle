...
const J = (E, e) => Object.fromEntries(Object.entries(E).filter(([r]) => !e.includes(r))), w = (E) => E !== Object(E), Q = (...E) => {
...
const u = (E) => E instanceof d, b = (E, e) => {
  let r = "";
  if (u(E) || E instanceof Error ? r = E.message : w(E) ? r = E : r = `${E.constructor.name} ${JSON.stringify(E)}`, E && (!E.statusCode || E.statusCode >= T.INTERNAL_SERVER_ERROR))
...
`, R] : []);
  ...
  const r = v().format(), R = w(E), i = R && E || E.message || "Internal Server Error", N = R && r || E.time || r, A = R && {} || E.details || {}, o = R && void 0 || E.code, _ = {
    statusCode: e,
    message: i,
    time: N,
    details: A,
...
const RE = (E, e) => {
  e.plugins && e.plugins.forEach(({ name: r, plugin: R }) => {
    ...
    R({ provider: E, log: i, extendRouterContext: rE, extendServiceContext: eE });
  });
}, nE = (E, e) => {
  ...
  i.forEach(function({ path: A, method: o, options: _ }) {
    const G = Q(r, A), H = D(`${$.bold(o)} ${G}`, "bgGreen");
    E.provider.createRoute({
      ...
      async handler(l) {
        ...
        try {
          ...
          const t = c?.statusCode || T.INTERNAL_SERVER_ERROR;
          return b(c, H), l.setStatusCode(t), h(c, t);
        }
      }
    });
  });
