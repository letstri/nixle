...
class Y extends Error {
  constructor({ statusCode: e, message: r, details: n, code: i }) {
    super(), this.time = d().format(), this.message = "Internal Server Error", Error.captureStackTrace(this, this.constructor), this.name = "NixleError", this.statusCode = e || T.BAD_REQUEST, this.message = r, this.details = n, this.code = i;
  }
}
const K = (E) => x({
  forError: E,
...
`, n] : []);
  ...
  const r = d().format(), n = V(E), i = n && E || E.message || "Internal Server Error", s = n && r || E.time || r, I = n && {} || E.details || {}, _ = n && void 0 || E.code, o = {
    statusCode: e,
    message: i,
    time: s,
    details: I,
...
const iE = (E, e) => {
  ...
  try {
    ...
    if (i.some(({ path: s, method: I, options: _ }) => !s || !I || !_.handler))
      throw l({
        message: "Path, method and handler are required for each route",
        statusCode: T.INTERNAL_SERVER_ERROR
      });
  } catch (s) {
    b(s, n), process.exit(1);
  }
  i.forEach(({ path: s, method: I, options: _ }) => {
    const o = p(r, s), F = D(`${w.bold(I)} ${o}`, "bgGreen");
    E.provider.createRoute({
      ...
      async handler(A) {
        ...
        try {
          e.guards.length && await Promise.all(e.guards.map((h) => h({ ...O, env: f }))), _?.guards?.length && await Promise.all(_.guards.map((h) => h({ ...O, env: f })));
        ...
        try {
          ...
          return m.emit("response", c), _?.statusCode && A.setStatusCode(_.statusCode), c;
        } catch (c) {
          const t = c?.statusCode || T.INTERNAL_SERVER_ERROR;
