import { createConsola as B } from "consola";
import { colorize as R, colors as w } from "consola/utils";
import Q from "dayjs";
import $ from "callsite-record";
import { joinURL as x } from "ufo";
import H from "mitt";
import q from "dotenv";
var T = /* @__PURE__ */ ((E) => (E[E.CONTINUE = 100] = "CONTINUE", E[E.SWITCHING_PROTOCOLS = 101] = "SWITCHING_PROTOCOLS", E[E.PROCESSING = 102] = "PROCESSING", E[E.EARLY_HINTS = 103] = "EARLY_HINTS", E[E.OK = 200] = "OK", E[E.CREATED = 201] = "CREATED", E[E.ACCEPTED = 202] = "ACCEPTED", E[E.NON_AUTHORITATIVE_INFORMATION = 203] = "NON_AUTHORITATIVE_INFORMATION", E[E.NO_CONTENT = 204] = "NO_CONTENT", E[E.RESET_CONTENT = 205] = "RESET_CONTENT", E[E.PARTIAL_CONTENT = 206] = "PARTIAL_CONTENT", E[E.MULTI_STATUS = 207] = "MULTI_STATUS", E[E.MULTIPLE_CHOICES = 300] = "MULTIPLE_CHOICES", E[E.MOVED_PERMANENTLY = 301] = "MOVED_PERMANENTLY", E[E.MOVED_TEMPORARILY = 302] = "MOVED_TEMPORARILY", E[E.SEE_OTHER = 303] = "SEE_OTHER", E[E.NOT_MODIFIED = 304] = "NOT_MODIFIED", E[E.USE_PROXY = 305] = "USE_PROXY", E[E.TEMPORARY_REDIRECT = 307] = "TEMPORARY_REDIRECT", E[E.PERMANENT_REDIRECT = 308] = "PERMANENT_REDIRECT", E[E.BAD_REQUEST = 400] = "BAD_REQUEST", E[E.UNAUTHORIZED = 401] = "UNAUTHORIZED", E[E.PAYMENT_REQUIRED = 402] = "PAYMENT_REQUIRED", E[E.FORBIDDEN = 403] = "FORBIDDEN", E[E.NOT_FOUND = 404] = "NOT_FOUND", E[E.METHOD_NOT_ALLOWED = 405] = "METHOD_NOT_ALLOWED", E[E.NOT_ACCEPTABLE = 406] = "NOT_ACCEPTABLE", E[E.PROXY_AUTHENTICATION_REQUIRED = 407] = "PROXY_AUTHENTICATION_REQUIRED", E[E.REQUEST_TIMEOUT = 408] = "REQUEST_TIMEOUT", E[E.CONFLICT = 409] = "CONFLICT", E[E.GONE = 410] = "GONE", E[E.LENGTH_REQUIRED = 411] = "LENGTH_REQUIRED", E[E.PRECONDITION_FAILED = 412] = "PRECONDITION_FAILED", E[E.REQUEST_TOO_LONG = 413] = "REQUEST_TOO_LONG", E[E.REQUEST_URI_TOO_LONG = 414] = "REQUEST_URI_TOO_LONG", E[E.UNSUPPORTED_MEDIA_TYPE = 415] = "UNSUPPORTED_MEDIA_TYPE", E[E.REQUESTED_RANGE_NOT_SATISFIABLE = 416] = "REQUESTED_RANGE_NOT_SATISFIABLE", E[E.EXPECTATION_FAILED = 417] = "EXPECTATION_FAILED", E[E.IM_A_TEAPOT = 418] = "IM_A_TEAPOT", E[E.INSUFFICIENT_SPACE_ON_RESOURCE = 419] = "INSUFFICIENT_SPACE_ON_RESOURCE", E[E.METHOD_FAILURE = 420] = "METHOD_FAILURE", E[E.MISDIRECTED_REQUEST = 421] = "MISDIRECTED_REQUEST", E[E.UNPROCESSABLE_ENTITY = 422] = "UNPROCESSABLE_ENTITY", E[E.LOCKED = 423] = "LOCKED", E[E.FAILED_DEPENDENCY = 424] = "FAILED_DEPENDENCY", E[E.UPGRADE_REQUIRED = 426] = "UPGRADE_REQUIRED", E[E.PRECONDITION_REQUIRED = 428] = "PRECONDITION_REQUIRED", E[E.TOO_MANY_REQUESTS = 429] = "TOO_MANY_REQUESTS", E[E.REQUEST_HEADER_FIELDS_TOO_LARGE = 431] = "REQUEST_HEADER_FIELDS_TOO_LARGE", E[E.UNAVAILABLE_FOR_LEGAL_REASONS = 451] = "UNAVAILABLE_FOR_LEGAL_REASONS", E[E.INTERNAL_SERVER_ERROR = 500] = "INTERNAL_SERVER_ERROR", E[E.NOT_IMPLEMENTED = 501] = "NOT_IMPLEMENTED", E[E.BAD_GATEWAY = 502] = "BAD_GATEWAY", E[E.SERVICE_UNAVAILABLE = 503] = "SERVICE_UNAVAILABLE", E[E.GATEWAY_TIMEOUT = 504] = "GATEWAY_TIMEOUT", E[E.HTTP_VERSION_NOT_SUPPORTED = 505] = "HTTP_VERSION_NOT_SUPPORTED", E[E.INSUFFICIENT_STORAGE = 507] = "INSUFFICIENT_STORAGE", E[E.NETWORK_AUTHENTICATION_REQUIRED = 511] = "NETWORK_AUTHENTICATION_REQUIRED", E))(T || {});
const W = (E, e) => Object.fromEntries(Object.entries(E).filter(([r]) => !e.includes(r))), G = (E) => E !== Object(E), M = (...E) => {
  const e = x("", ...E), r = e.startsWith("/") ? e : `/${e}`;
  return r.endsWith("/") ? r.slice(0, -1) : r;
}, F = (E) => {
  try {
    const e = JSON.parse(E);
    return typeof e == "number" || typeof e == "boolean" || e === void 0 || e === null ? e : E;
  } catch {
    return E;
  }
}, p = (E) => Object.fromEntries(
  Object.entries(E).map(([e, r]) => [
    e,
    Array.isArray(r) ? r.map(F) : F(r)
  ])
), f = H(), k = {
  syntax: {
    string: (...E) => R("green", E.join("")),
    punctuator: (...E) => R("gray", E.join("")),
    keyword: (...E) => R("cyan", E.join("")),
    number: (...E) => R("magenta", E.join("")),
    regex: (...E) => R("magenta", E.join("")),
    comment: (...E) => R("gray", R("bold", E.join(""))),
    invalid: (...E) => R("inverse", E.join(""))
  },
  codeFrame: (E) => E.slice(1),
  codeLine(E, e, r, n) {
    let N = (e ? " > " : "   ") + R("dim", E) + " ";
    e && (N = R("bgRed", N));
    let I = N + R("dim", "| ") + r;
    return n || (I += `
`), I;
  },
  stackLine(E, e, r) {
    let n = `   ${R("dim", "at")} ` + E + " (" + R("blueBright", R("underline", e)) + ")";
    return r || (n += `
`), n;
  },
  stack(E) {
    return `

` + E;
  }
};
class V extends Error {
  constructor({ statusCode: e, message: r, details: n, code: i }) {
    super(), this.time = Q().format(), this.message = "Internal Server Error", Error.captureStackTrace(this, this.constructor), this.name = "NixleError", this.statusCode = e || T.BAD_REQUEST, this.message = r, this.details = n, this.code = i;
  }
}
const X = (E) => $({
  forError: E,
  isCallsiteFrame: (r) => h(E) && E.statusCode < T.INTERNAL_SERVER_ERROR ? !!r.source && !r.source.includes("node_modules") && !r.source.includes("node:") && !r.source.includes("nixle/dist") : !0
})?.renderSync({
  renderer: k,
  stackFilter: (r) => h(E) && E.statusCode < T.INTERNAL_SERVER_ERROR ? !!r.source && !r.source.includes("node_modules") && !r.source.includes("node:") && !r.source.includes("nixle/dist") : !0
});
function o(E, e) {
  const r = typeof E == "string" ? E : E.message;
  return new V({
    message: r,
    statusCode: typeof E == "string" ? e || T.BAD_REQUEST : E.statusCode || T.BAD_REQUEST,
    code: typeof E == "string" ? void 0 : E.code,
    details: typeof E == "string" ? {} : E.details || {}
  });
}
const h = (E) => E instanceof V, u = (E, e) => {
  let r = "";
  if (h(E) || E instanceof Error ? r = E.message : G(E) ? r = E : r = `${E.constructor.name} ${JSON.stringify(E)}`, E && (!E.statusCode || E.statusCode >= T.INTERNAL_SERVER_ERROR))
    if (E instanceof Error) {
      const n = X(E);
      e.fatal(R("red", r), ...n ? [`
`, n] : []);
    } else
      e.fatal(R("red", r));
  else
    e.error(R("red", r));
  f.emit("error", E);
}, g = (E, e) => {
  const r = Q().format(), n = G(E), i = n && E || E.message || "Internal Server Error", N = n && r || E.time || r, I = n && {} || E.details || {}, _ = n && void 0 || E.code, l = {
    statusCode: e,
    message: i,
    time: N,
    details: I,
    code: _
  };
  return l.details = {
    ...l.details,
    ...W(JSON.parse(JSON.stringify(E, Object.getOwnPropertyNames(E))), [
      "message",
      "name",
      "stack",
      "statusCode",
      "time",
      "details",
      "code"
    ])
  }, l;
};
let y;
const K = (E) => {
  y = B(E);
}, t = (E, ...e) => {
  if (!y)
    return;
  const r = `${R("bgBlue", " Nixle ")}`, n = y[E];
  if (!n)
    throw o({
      message: `Logger method "${E}" not found`,
      statusCode: T.INTERNAL_SERVER_ERROR
    });
  n(`${r}`, ...e);
}, b = {
  info: (...E) => t("info", ...E),
  success: (...E) => t("success", ...E),
  warn: (...E) => t("warn", ...E),
  error: (...E) => t("error", ...E),
  fatal: (...E) => t("fatal", ...E),
  debug: (...E) => t("debug", ...E),
  trace: (...E) => t("trace", ...E),
  silent: (...E) => t("silent", ...E),
  log: (...E) => t("log", ...E),
  fail: (...E) => t("fail", ...E),
  verbose: (...E) => t("verbose", ...E)
}, m = (E, e = "bgWhite") => Object.fromEntries(
  Object.entries(b).map(([r, n]) => [
    r,
    (...i) => n(R(e, ` ${E} `), ...i)
  ])
);
function J(E, e) {
  const r = {};
  return typeof e == "function" ? {
    path: E,
    method: "GET",
    options: {
      handler: e
    },
    $infer: r
  } : {
    path: E,
    method: "GET",
    options: e,
    $infer: r
  };
}
function Z(E, e) {
  const r = {};
  return typeof e == "function" ? {
    path: E,
    method: "POST",
    options: {
      handler: e
    },
    $infer: r
  } : {
    path: E,
    method: "POST",
    options: e,
    $infer: r
  };
}
function z(E, e) {
  const r = {};
  return typeof e == "function" ? {
    path: E,
    method: "PATCH",
    options: {
      handler: e
    },
    $infer: r
  } : {
    path: E,
    method: "PATCH",
    options: e,
    $infer: r
  };
}
function S(E, e) {
  const r = {};
  return typeof e == "function" ? {
    path: E,
    method: "PUT",
    options: {
      handler: e
    },
    $infer: r
  } : {
    path: E,
    method: "PUT",
    options: e,
    $infer: r
  };
}
function C(E, e) {
  const r = {};
  return typeof e == "function" ? {
    path: E,
    method: "DELETE",
    options: {
      handler: e
    },
    $infer: r
  } : {
    path: E,
    method: "DELETE",
    options: e,
    $infer: r
  };
}
function EE(E, e) {
  const r = {};
  return typeof e == "function" ? {
    path: E,
    method: "OPTIONS",
    options: {
      handler: e
    },
    $infer: r
  } : {
    path: E,
    method: "OPTIONS",
    options: e,
    $infer: r
  };
}
const eE = {
  get: J,
  post: Z,
  patch: z,
  put: S,
  delete: C,
  options: EE
}, D = {}, rE = (E) => {
  q.config(E), Object.keys(process.env).forEach((e) => {
    D[e] = process.env[e];
  });
}, d = {}, nE = (E) => {
  Object.assign(d, E);
};
function AE(E, e) {
  const r = typeof e == "object";
  if (!e || r && !e.routes)
    throw o("Routes are required", T.INTERNAL_SERVER_ERROR);
  const n = r ? e.routes : e, i = r ? e.guards || [] : [];
  return {
    path: E,
    routes: () => n({
      route: eE,
      log: m(E, "bgGreen"),
      env: D,
      ...d
    }),
    guards: i,
    $inferRoutes: {}
  };
}
let Y = {};
const RE = (E) => {
  Object.assign(Y, E);
};
function oE(E, e) {
  function r() {
    return e({
      log: m(E, "bgCyan"),
      env: D,
      ...Y
    });
  }
  return r;
}
const TE = (E, e) => {
  e.plugins && e.plugins.forEach(({ name: r, plugin: n }) => {
    const i = m(r, "bgMagenta");
    n({ provider: E, log: i, extendRouterContext: nE, extendServiceContext: RE });
  });
}, iE = (E, e) => {
  const r = M(E.globalPrefix || "", e.path || ""), n = m(r, "bgGreen"), i = e.routes();
  try {
    if (i.length === 0)
      throw o({
        message: "At least one router is required",
        statusCode: T.INTERNAL_SERVER_ERROR
      });
    if (i.some(({ path: N, method: I, options: _ }) => !N || !I || !_.handler))
      throw o({
        message: "Path, method and handler are required for each route",
        statusCode: T.INTERNAL_SERVER_ERROR
      });
  } catch (N) {
    u(N, n), process.exit(1);
  }
  i.forEach(({ path: N, method: I, options: _ }) => {
    const l = M(r, N), j = m(`${w.bold(I)} ${l}`, "bgGreen");
    E.provider.createRoute({
      method: I.toLowerCase(),
      path: l,
      async handler(A) {
        f.emit("request", A);
        const s = {
          ...A,
          query: p(A.query),
          params: p(A.params)
        };
        try {
          await _?.middleware?.(s);
        } catch (c) {
          const O = c?.statusCode || T.INTERNAL_SERVER_ERROR;
          return A.setStatusCode(O), g(c, O);
        }
        let U = s.query, L = s.params, a = s.body;
        try {
          e.guards.length && await Promise.all(e.guards.map((P) => P({ ...s, env: D }))), _?.guards?.length && await Promise.all(_.guards.map((P) => P({ ...s, env: D })));
          const [c, O, v] = await Promise.all([
            _?.queryValidation?.(s.query),
            _?.paramsValidation?.(s.params),
            _?.bodyValidation?.(s.body)
          ]);
          U = c || U, L = O || L, a = v || a;
        } catch (c) {
          const O = c?.statusCode || T.BAD_REQUEST;
          return A.setStatusCode(O), g(c, O);
        }
        try {
          const c = await _.handler({
            ...s,
            query: U,
            params: L,
            body: a
          });
          return f.emit("response", c), _?.statusCode && A.setStatusCode(_.statusCode), c;
        } catch (c) {
          const O = c?.statusCode || T.INTERNAL_SERVER_ERROR;
          return u(c, j), A.setStatusCode(O), g(c, O);
        }
      }
    });
  });
};
function lE(E) {
  E.logger !== !1 && K(E.logger || {});
  try {
    if (!E.provider)
      throw o("Provider is required", T.INTERNAL_SERVER_ERROR);
    if (E.routers.length === 0)
      throw o("At least one router is required", T.INTERNAL_SERVER_ERROR);
  } catch (r) {
    u(r, b), process.exit(1);
  }
  E.plugins && TE(E.provider, E), rE(E.env), E.routers.forEach((r) => {
    iE(E, r);
  }), E.provider.globalMiddleware(({ setHeader: r }) => {
    r("X-Powered-By", "Nixle");
  });
  const e = {
    app: E.provider.app,
    events: {
      on: f.on,
      emit: f.emit
    },
    $inferRouters: {}
  };
  return b.success("🔥 Application successfully started"), e;
}
function fE(E, e) {
  return async (r) => {
    try {
      await e(r);
    } catch (n) {
      throw o({
        message: n?.message || `Oops, ${E} guard was failed`,
        statusCode: n?.statusCode || T.BAD_REQUEST,
        details: n?.details
      });
    }
  };
}
function mE(E) {
  return E;
}
const DE = (E, e) => ({
  name: E,
  plugin: e
});
export {
  T as StatusCode,
  lE as createApp,
  o as createError,
  fE as createGuard,
  DE as createPlugin,
  mE as createProvider,
  AE as createRouter,
  oE as createService,
  nE as extendRouterContext,
  h as isNixleError
};
