...
class G extends Error {
  constructor({ statusCode: e, message: r, details: n }) {
    super(), this.time = M().format(), this.message = "Internal Server Error", Error.captureStackTrace(this, this.constructor), this.name = "NixleError", this.statusCode = e || s.BAD_REQUEST, this.message = r, this.details = n;
  }
}
const $ = (E) => Y({
  forError: E,
...
const eE = (E, e) => {
  e.plugins && e.plugins.forEach(({ name: r, plugin: n }) => {
    const _ = U(r, "bgMagenta");
    n({ provider: E, log: _, extendRouterContext: C, extendServiceContext: EE }), g.success(`🚀 ${c("bgBlue", ` ${r.trim()} `)} plugin successfully loaded`);
  });
}, rE = (E, e) => {
  const r = y(E.globalPrefix || "", e.path || ""), n = U(r, "bgGreen"), _ = e.routes();
  try {
    _.length === 0 && t({
  ...
  _.forEach(({ path: T, method: N, options: R }) => {
    const m = y(r, T), o = U(`${j.bold(N)} ${m}`, "bgGreen");
    E.provider.createRoute({
      ...
      async handler(I) {
        ...
        try {
          e.guards.length && await Promise.all(e.guards.map((i) => i({ ...l, env: f }))), R?.guards?.length && await Promise.all(R.guards.map((i) => i({ ...l, env: f }))), await Promise.all([
        ...
        try {
          ...
          const A = i?.statusCode || s.INTERNAL_SERVER_ERROR;
          return h(i, o), I.setStatusCode(A), L(i, A);
        }
      }
